...
if(!class_exists('WPvivid_Tab_Page_Container'))
    include_once WPVIVID_IMGOPTIM_DIR . '/includes/class-wpvivid-tab-page-container.php';
...
class WPvivid_Tab_Page_Container_Ex extends WPvivid_Tab_Page_Container
{
    ...
    public function display_tabs()
    {
        ...
        foreach ($this->tabs as $tab)
        {
            ...
            if(isset($tab['can_delete']))
            {
                echo '<span class="'.esc_attr($span_class).'" style="'.esc_attr($span_style).'"></span><span style="padding-right: 1em;">'.esc_html($tab['title']).'</span>';
            ...
            {
                echo '<span class="'.esc_attr($span_class).'" style="'.esc_attr($span_style).'"></span><span>'.esc_html($tab['title']).'</span>';
            }
            echo '</a>';
        }
    }
