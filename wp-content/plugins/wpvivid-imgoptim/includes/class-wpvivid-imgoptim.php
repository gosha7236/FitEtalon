...
class WPvivid_ImgOptim
{
    ...
    public function add_action_links( $links )
    {
        ...
        if($has_addons)
        {
            ...
            $settings_link = array(
                '<a href="' . admin_url($url).'">' . __('Settings', WPVIVID_IMGOPTIM_SLUG) . '</a>',
            );
        }
        else
        {
    ...
    public function submitbox()
    {
        ...
        if(get_option('wpvivid_imgoptim_user',false)===false)
        {
            ...
            echo '<div class="misc-pub-section misc-pub-wpvivid"><h4>'.__('WPvivid Imgoptim','wpvivid-imgoptim').'</h4>';
            echo '<p>'.__('Not set License','wpvivid-imgoptim').'</p>';
            echo '<a href="'.esc_url($url).'">'.__('Check your Settings','wpvivid-imgoptim').'</a>';
            echo '</div>';
        }
        else
        {
            ...
            {
                echo  __('Not support','wpvivid-imgoptim');
            }
            else
            {
                $meta=get_post_meta( $post->ID,'wpvivid_image_optimize_meta', true );
                echo '<div class="misc-pub-section misc-pub-wpvivid" data-id="'.esc_attr($post->ID).'"><h4>'.__('WPvivid Imgoptim','wpvivid-imgoptim').'</h4>';
                ...
                if(!$task->is_image_optimized($post->ID))
                {
                    ...
                    {
                        echo "<a  class='wpvivid-media-progressing button-primary' data-id='".esc_attr($post->ID)."'>".__('Optimizing...','wpvivid-imgoptim')."</a>";
                    }
                    else
                    {
                        echo "<a  class='wpvivid-media button-primary' data-id='".esc_attr($post->ID)."'>".__('Optimize','wpvivid-imgoptim')."</a>";
                    }
                }
                else
                {
                    ...
                    echo '<ul>';
                    echo '<li><span>'.__('Optimized size','wpvivid-imgoptim').' : </span><strong>'.esc_html(size_format($meta['sum']['opt_size'],2)).'</strong></li>';
                    echo '<li><span>'.__('Saved','wpvivid-imgoptim').' : </span><strong>'.esc_html($percent).'%</strong></li>';
                    echo '<li><span>'.__('Original size','wpvivid-imgoptim').' : </span><strong>'.esc_html(size_format($meta['sum']['og_size'],2)).'</strong></li>';
                    echo "<li><a  class='wpvivid-media-restore button-primary' data-id='".esc_attr($post->ID)."'>".__('Restore','wpvivid-imgoptim')."</a></li>";
                    echo '</ul>';
                }
