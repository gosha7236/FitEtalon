...
Stripe.setPublishableKey('pk_test_nddHOsaIrUVSkmrzYC9Gt1nY');
$(document).ready(function() {
	...
	//2
	 $("#EnablepaymentForm").submit(function(event) {
        $('#EnableCompetitorReg').attr("disabled", "disabled");
        // create stripe token to make payment
        Stripe.createToken({
            number: $('#cardNumber').val(),
            cvc: $('#cardCVC').val(),
...
// handle the response from stripe
function handleStripeResponseNew(status, response) {
	console.log(JSON.stringify(response));
    if (response.error) {
        $('#EnableCompetitorReg').removeAttr("disabled");
        $(".paymentErrors").html(response.error.message);
    } else {
		var payForm = $("#EnablepaymentForm");
        //get stripe token id from response
        var stripeToken = response['id'];
        //set the token into the form hidden input to make payment
        payForm.append("<input type='hidden' name='stripeToken' value='" + stripeToken + "' />");
